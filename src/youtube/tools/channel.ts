...
Description: ${channelInfo.snippet.description.slice(0, 200)}${channelInfo.snippet.description.length > 200 ? '...' : ''}`
  ...
  // Get list of channel videos with optional filtering
  server.tool(
    "get_channel_videos",
    "Get list of channel videos with optional filters for demographic analysis",
    {
    ...
    async ({ query, startDate, endDate, maxResults = 25 }) => {
      try {
        const youtubeClient = await getYouTubeClient();
